...
def qwen2_vl_forward(
    ...
    **kwargs,
    ...
    # 2) If still None, build true mRoPE indices like the old code did
    if norm_position_ids is None and (attention_mask is None or attention_mask.ndim == 2):
        # Try to use the model's own helper if available (Qwen2/VL has it similar to Qwen2.5)
        try:
            pid_3ch, rope_deltas = self.get_rope_index(
                input_ids,
                image_grid_thw,
                video_grid_thw,
            ...
            # Your normalizer will add the text channel for >=4.54
            norm_position_ids = process_position_ids(pid_3ch, attention_mask=attention_mask)
            # Optional: keep rope_deltas if you also want decode continuity
            self.rope_deltas = rope_deltas
        except AttributeError:
            # Fallback: at least don't fabricate vision channels from text;
            # require the caller to pass proper 4-channel ids for multimodal batches.
            pass
