...
def gui_action_match_compute_score(solution_str, ground_truth, extra_info=None):
    ...
    try:
        ...
        if 'action_content' in result and result['action_content'] is not None:
            pred_action = result['action_content']
            type_match, extract_match, weight = check_response_match(pred_action, ground_truth, width, height, resized_width, resized_height)
            if type_match:
                action_score = 0.5
                if extract_match:
                    action_score = 1.0
    ...
    if solution_str.startswith("<action>") and solution_str.endswith("</action>") and action_score > 0:
        format_score = 1 
    else:
        format_score = 0

    # format_score = 1 if action_score > 0 else 0
    # print(f"action_score {action_score}")
    return {
