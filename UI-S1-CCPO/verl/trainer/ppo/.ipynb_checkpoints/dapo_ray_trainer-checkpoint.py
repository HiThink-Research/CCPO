            ...
            for batch_dict in self.train_dataloader:
                ...
                with _timer("step", timing_raw):
                    ...
                    with _timer("gen", timing_raw):
                        ...
                        # mr_gen.save_messages_to_json("messages.json")
                        
                    # print("开始检查 step_batch_list 中每个 DataProto 对象的状态:")
                    # for idx, data_proto in enumerate(step_batch_list):
                    #     print(f"--- DataProto {idx} 的 non_tensor_batch 信息 ---")
                    #     if hasattr(data_proto, 'non_tensor_batch') and data_proto.non_tensor_batch is not None:
                    #         for key, value in data_proto.non_tensor_batch.items():
                    #             # 打印每个字段的键、类型和形状（如果是数组）或长度（如果是列表等）
                    #             if hasattr(value, 'shape'):
                    #                 print(f"  键 '{key}': 类型={type(value)}, 形状={value.shape}")
                    #             elif hasattr(value, '__len__'):
                    #                 print(f"  键 '{key}': 类型={type(value)}, 长度={len(value)}")
                    #             else:
                    #                 print(f"  键 '{key}': 类型={type(value)}, 值={value}")
                    #     else:
                    #         print("   non_tensor_batch 为 None 或不存在")
                    #     print()  # 空行分隔
