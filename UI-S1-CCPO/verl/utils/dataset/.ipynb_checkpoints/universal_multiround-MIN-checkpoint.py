...
class QwenMessages2Inputs():
    def __init__(self, tokenizer: PreTrainedTokenizer, config: DictConfig, processor: Any | None = None):
        ...
        self.num_image_limit = config.get("num_image_limit", 3)
    ...
    def _apply_bboxes_to_images(self, images, bboxes):
        """
        images : List[PIL.Image.Image]
        bboxes : List[List[float]]  # [[x1,y1,x2,y2], ...], one per image.
                                    # Can be pixel coords or normalized in [0,1].
        returns: List[PIL.Image.Image] (cropped)
        """
        if bboxes is None:
            return images
        ...
        for img, box in zip(images, bboxes):
            # Allow None/empty box to mean "no crop"
            ...
            if 0.0 <= min(x1, y1, x2, y2) <= 1.0 and max(x1, y1, x2, y2) <= 1.0:
                x1, x2 = x1 * w, x2 * w
                y1, y2 = y1 * h, y2 * h
            ...
            if (x2 - x1) < 1 or (y2 - y1) < 1:
                cropped.append(img)  # fall back to original
            else:
                cropped.append(img.crop((x1, y1, x2, y2)))
        
        # print(f"images: {cropped}")
        return cropped
    ...
    def __call__(self, state):
        ...
        # print("+++++"*20)
        # print(f"DEBUG: ptr={state['ptr']}, example_group={state['example_id']}, rollout_id={state['rollout_id']}")
        # print("previous_aggregate_coordinates")
        # print(state["previous_aggregate_coordinates"])
        # print("current response")
        # print(state["cur_resp"])
        # print("previous reference")
        # print(state["pre_ref"])
        # print("previous response")
        # print(state["pre_resp"])
        ...
        if len(bboxes) >= len(image_inputs):
            ...
            print(f"DEBUG: ptr={state['ptr']}, example_group={state['example_id']}, rollout_id={state['rollout_id']}")
            print(state["previous_aggregate_coordinates"])
            # print(state["previous_aggregate_coordinates_later"])
            # print(f"rollout {state['rollout_n']}")
            # print(state["cur_resp"])
            # print(state["pre_ref"])
            # print(state["pre_resp"])
            print(f"bboxes {bboxes}")
        ...
        if bboxes is not None:
            ...
            image_inputs = self._apply_bboxes_to_images(image_inputs, bboxes)
        ...
        # print([str(i) for i in image_inputs])
        # row_dict["images"] = [str(i) for i in image_inputs]
...
class MultiRoundGenerator():
    ...
    def _fetch_next(self, ptr):
        ...
        # print(f"Ptr {ptr} - Previous Coordinate{len(self.previous_aggregate_coordinates)}")
        # Get coordinates for the example group this ptr belongs to
        state['previous_img_bbox'] = [self.coords_to_bbox(coords) for coords in self.previous_aggregate_coordinates[example_group]].copy()
        state['previous_img_bbox'].append([])
        state['previous_img_bbox'] = state['previous_img_bbox']
        # print(f"Ptr {ptr} - Previous bbox: {state['previous_img_bbox']}")
        # print("@@@"*40)
        
        # Get aggregate coordinates for the example group
        state["previous_aggregate_coordinates"]= copy.deepcopy(self.previous_aggregate_coordinates[example_group])
        state["previous_aggregate_coordinates_later"]= self.previous_aggregate_coordinates[example_group]
        # state['cur_resp'] = self.current_response
        # state["pre_ref"] = self.get_all_previous_ref_responses()
        # state["pre_resp"] = self.previous_response
        
        row_dict = self.msg_man(state)
        # print(state['messages'])
        # print(row_dict["images"])

        # Collect messages for JSON logging instead of printing
        
        # self._save_message(ptr, {
        #     "input_data": row_dict["raw_prompt"],
        #     "multi_modal_inputs" :row_dict["images"],
        #     "previous_aggregate_coordinates": self.previous_aggregate_coordinates[example_group],
        #     # "previous_img_bbox": state['previous_img_bbox'],
        #     "cur_resp": self.current_response,
        #     "pre_ref": self.get_all_previous_ref_responses(),
        #     "pre_resp": self.previous_response,  
        #     'bbox': state['previous_img_bbox']
        # })
        
        # del row_dict["raw_prompt"]
        # del row_dict["images"]
        ...
        while True:
            ...
            for ptr, res in enumerate(mid_result):
                ...
                if row_dict == "Finished":
                    ...
                    # self.reset_previous_responses(ptr)
                    # self.reset_previous_ref_responses(ptr)
                    # self.reset_previous_aggregate_coordinates(ptr)
                else:
            ...
            if len(batch) == 0:
                ...
                # print("prev response")
                # for i in self.previous_response:
                #     print(i) 
                # print("prev ref response")
                # for i in self.previous_ref_response:
                #     print(i) 
                    
                # self.reset_previous_aggregate_coordinates()
                # self.reset_all_previous_ref_responses()
                # self.reset_all_previous_responses()
                break
    ...
    def apply_response(self, batch):
        ...
        for ptr, response, extract_match, reward_model,extra_info in zip(batch.non_tensor_batch['ptr'], batch.batch['responses'], batch.non_tensor_batch['extract_match'], batch.non_tensor_batch['reward_model'], batch.non_tensor_batch['extra_info']):
            ...
            # if self.current_response[ptr] is not None:
            #     model_response = self.fm.parse_response(self.current_response[ptr])
            #     self.previous_response[ptr].append(model_response['action_content'])
    ...
    def reset_previous_aggregate_coordinates(self, ptr=None):
        ...
        """
        # print(f"{ptr} |{ptr // self.rollout_n} has been reset_previous_aggregate_coordinates")
        if ptr is not None:
            example_group = ptr // self.rollout_n
    ...
    def aggregate_previous_coordinates(self, ptr):
        ...
        # Example group has rollout_n trajectories starting at example_group * rollout_n
        ...
        cur_round = min([len(pr[i]) for i in index])
        
        # print(f"Group: {example_group}, prr_idx: {prr_idx}, round:{cur_round}")
        ...
        # print(f"pr len: {len(pr)}")
        # print(f"prr len: {len(prr)}")
        # print(aggregate_coords)    

        # if all(x == prr_len[0] for x in prr_len): 
            # self._previous_aggregate_keys[example_group] = set()
        ...
        # only store if this aggregate hasn't been seen before for this example group
        # if key not in self._previous_aggregate_keys[example_group]:
        #     self.previous_aggregate_coordinates[example_group].append(aggregate_coords)
        #     self._previous_aggregate_keys[example_group].add(key)
        ...
        if len(self.previous_aggregate_coordinates[example_group]) < cur_round:
            ...
            self.previous_aggregate_coordinates[example_group].append(aggregate_coords)
            # while len(self.previous_aggregate_coordinates[example_group]) < cur_round:
                # time.sleep(0.5)
                # print(f"round:{cur_round} group {example_group} {len(self.previous_aggregate_coordinates[example_group])}")
            # print(f"round:{cur_round} group {example_group} {len(self.previous_aggregate_coordinates[example_group])}")
            # print(len(self.previous_aggregate_coordinates[example_group]))
            
            # self._previous_aggregate_keys[example_group].add(key)
                
        # tmp_len = len(self.previous_aggregate_coordinates[example_group])
        # self.previous_aggregate_coordinates[example_group].append(aggregate_coords)
        # assert len(self.previous_aggregate_coordinates[example_group]) == tmp_len + 1
...
if __name__ == "__main__":
    ...
    for sub_batch in mr_gen.fetch_batch():
        ...
        # Print raw response text for each item
        ...
        for ptr in sub_batch.non_tensor_batch['ptr']:
            ...
            # mr_gen.current_response[ptr] = '<think>\nxxx\n</think>\n<action>\n{\"action\": \"click_'+ str(i) +'\", \"coordinate\": [670, 2060]}\n</action>'
            mr_gen.current_response[ptr] = '<think>\nxxx\n</think>\n<action>\n{\"action\": \"click' +'\", \"coordinate\": [2'+ str(ptr)+', 6'+ str(i)+']}\n</action>'
            # mr_gen.current_response[ptr] = '<action>\n{\"action\": \"open' +'\", \"text\": \"Amazon\" }\n</action>'
            ...
            # # Print response history to demonstrate the new functionality
            # print(f"Ptr {ptr} - Previous responses: {mr_gen.get_previous_responses(ptr)}")
            # print(f"Ptr {ptr} - Current response: {mr_gen.current_response[ptr]}")

            # print("@@@"*40)
            # mr_gen.aggregate_previous_coordinates()
            # print(f"Ptr {ptr} - Previous responses: {mr_gen.get_all_previous_responses()}")
            # print("--"*40)
            # print(f"Ptr {ptr} - Previous Ref responses: {mr_gen.get_all_previous_ref_responses()}")
            # print("--"*40)
            # print(f"Ptr {ptr} - Previous Coordinates: {mr_gen.previous_aggregate_coordinates}")
            # print("@@@"*40)
        i+=1

    # json_filename = mr_gen.save_messages_to_json("messages.json")
    # print(f"All messages have been saved to: {json_filename}")
            ## calculate reward
