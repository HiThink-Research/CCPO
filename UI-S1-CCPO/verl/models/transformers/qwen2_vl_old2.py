...
#     **kwargs,
def ulysses_flash_attn_forward(self, *f_args, **f_kwargs):
    from transformers.models.qwen2_vl.modeling_qwen2_vl import apply_multimodal_rotary_pos_emb, repeat_kv

        # ---- extract canonical arguments without causing duplicate-value errors ----
    ...
    hidden_states      = _get_arg(0, "hidden_states")                 # required
    attention_mask     = _get_arg(1, "attention_mask", None)
    position_ids       = _get_arg(2, "position_ids", None)

    # Optional / extra args that HF may pass in recent versions
    position_embeddings = f_kwargs.pop("position_embeddings", None)
    cache_position      = f_kwargs.pop("cache_position", None)        # unused here
    past_key_value      = f_kwargs.pop("past_key_value", None) or f_kwargs.pop("past_key_values", None)
    output_attentions   = f_kwargs.pop("output_attentions", False)    # we return None weights
    use_cache           = f_kwargs.pop("use_cache", None)             # unused in attn compute

    # Any remaining args in f_kwargs are ignored safely, but we keep them for FA2 call if useful
    # ----------------------------------------------------------------------------
    ...
    # (B, L, H*D) -> (B, H, L, D) for Q; and KV with num_key_value_heads
    ...
    if ulysses_sp_size > 1:
        ...
        # expand kv heads and shard sequence across heads
    ...
    # 'mrope_section' exists on Qwen2-VL attention modules
    query_states, key_states = apply_multimodal_rotary_pos_emb(
        query_states, key_states, cos, sin, self.rope_scaling["mrope_section"]
    )

    # prepare for FlashAttention: (B, H, L, D) -> (B, L, H, D)
    ...
    # Maintain HF attention return contract: (attn_output, attn_weights)
    # Most Qwen2-VL paths expect (output, None, None) from FA2-style modules.
