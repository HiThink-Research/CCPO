...
def apply_monkey_patch(
    ...
    if model.config.model_type == "qwen2_5_vl":
        ...
        # Only when you need the FA2-specific Ulysses forward
        if use_remove_padding or ulysses_sp_size > 1:
            from verl.models.transformers.qwen2_vl import ulysses_flash_attn_forward

            try:
                # <= transformers 4.52.x: class exists
                from transformers.models.qwen2_5_vl.modeling_qwen2_5_vl import (
                    Qwen2_5_VLFlashAttention2 as _Qwen25VLTextAttnFA2,
                )
                _Qwen25VLTextAttnFA2.forward = ulysses_flash_attn_forward
                print("Monkey patched Qwen2.5-VL FlashAttention2.forward (<=4.52.x)")
            except Exception:
                # >= transformers 4.53: FA2 is selected inside the unified attention classes
                from transformers.models.qwen2_5_vl.modeling_qwen2_5_vl import (
                    Qwen2_5_VLAttention as _Qwen25VLTextAttn,
                )
                # Patch text attention forward (covers FA2 path in >=4.53)
                _Qwen25VLTextAttn.forward = ulysses_flash_attn_forward
                # If you also need to patch the vision attention, do it conditionally:
                try:
                    from transformers.models.qwen2_5_vl.modeling_qwen2_5_vl import (
                        Qwen2_5_VLVisionAttention as _Qwen25VLVisionAttn,
                    )
                    # Only patch if your ulysses impl supports the vision forward signature.
                    # _Qwen25VLVisionAttn.forward = ulysses_flash_attn_forward
                except Exception:
                    pass
                print("Monkey patched Qwen2.5-VL attention.forward (>=4.53)")

        # Your existing input-slicing patch is still correct:
        if ulysses_sp_size > 1:
